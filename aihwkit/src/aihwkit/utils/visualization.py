...
def plot_device_compact(
    ...
    **plot_kwargs: Any,
    ...
    if "linewidth" not in plot_kwargs:
        plot_kwargs["linewidth"] = 2  # 稍微加粗主线
    axes.plot(w_trace, **plot_kwargs)
    axes.plot(
        np.arange(len(w_trace)),  # x轴: 0,1,2,3,...
        w_trace[:, 0],            # y轴: conductance值
        marker='o',               # 标记形状: 圆圈
        markersize=6,             # 圆圈放大
        linestyle='None',         # 不连线，只画点
        color='blue'              # 颜色，和主线一致
    )
    ...
    axes.annotate('', xy=(x_pos, g1), xytext=(x_pos, g0),
                arrowprops=dict(arrowstyle='<->', color='red', linewidth=1.5))
    axes.text(x_pos + 0.3, (g0 + g1) / 2, r'$\Delta w^{p}_{\min}$',
            color='red', fontsize=25, va='center')

    axes.set_title(analog_tile.rpu_config.device.__class__.__name__, fontsize=14)
    axes.set_xlabel("Number of Pulses", fontsize=15)
    axes.set_ylabel("Weight", fontsize=15)
    axes.set_ylim(-1.05, 1.05)
    axes.set_xlim(0, total_iters - 1)
    axes.axhline(y=1.0, color='gray', linestyle='--', linewidth=2)
    axes.axhline(y=-1.0, color='gray', linestyle='--', linewidth=2)
    axes.text(len(w_trace) * 0.9, 0.75, r'$\tau_{\mathrm{max}}$', ha='right', va='bottom', fontsize=25)
    axes.text(len(w_trace) * 0.2, -0.75, r'$\tau_{\mathrm{min}}$', ha='right', va='top', fontsize=25)
    ...
    axes.ticklabel_format(style='plain', axis='x')  # 禁用科学记数法
    axes.tick_params(labelsize=11)  # 放大刻度标签字体
