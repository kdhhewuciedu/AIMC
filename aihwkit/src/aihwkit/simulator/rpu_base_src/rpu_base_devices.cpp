...
template <typename T> void declare_rpu_devices(py::module &m, std::string type_name_add) {
  ...
  py::class_<RPU::PulsedUpdateMetaParameter<T>>(m, NAME("AnalogTileUpdateParameter"))
      ...
      .def_readwrite("trigger_tile_switch", &RPU::PulsedUpdateMetaParameter<T>::trigger_tile_switch);
  ...
  py::class_<RPU::AbstractRPUDevice<T>>(m, ("AbstractRPUDevice" + type_name_add).c_str());

  py::class_<RPU::PulsedRPUDeviceBase<T>, RPU::AbstractRPUDevice<T>>(m, ("PulsedRPUDeviceBase" + type_name_add).c_str());
  
  py::class_<RPU::VectorRPUDevice<T>, RPU::PulsedRPUDeviceBase<T>>(m, ("VectorRPUDevice" + type_name_add).c_str());
  
  py::class_<RPU::TransferRPUDevice<T>, RPU::VectorRPUDevice<T>>(m, ("TransferRPUDevice" + type_name_add).c_str())
    .def("get_has_switched_tile", &RPU::TransferRPUDevice<T>::getHasSwitchedTile)
    .def("set_has_switched_tile", &RPU::TransferRPUDevice<T>::setHasSwitchedTile);
