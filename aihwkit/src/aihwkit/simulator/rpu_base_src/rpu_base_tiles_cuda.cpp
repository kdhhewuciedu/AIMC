...
void declare_rpu_tiles_cuda(py::module &m, std::string type_name_add, bool add_utilities) {
  ...
  py::class_<ClassPulsed, RPU::RPUCudaSimple<T_RPU>>(
    ...
    Args:
      ...
      // .def(  
      //   "set_flags",
      //   [](ClassPulsed &self, bool trigger_tile_switch_flag) {
      //       std::lock_guard<std::mutex> lock(self.mutex_);
      //       self.getRPUDeviceCuda().set_flags_cpp(trigger_tile_switch_flag);
      //   },
      //   py::arg("trigger_tile_switch_flag"),
      //   R"pbdoc(
      //       Set internal control flags (e.g. trigger_tile_switch) during training.

      //       Args:
      //           trigger_tile_switch_flag: whether to trigger tile switching
      //   )pbdoc");
      .def(
        "set_flags",
        [](ClassPulsed &self, bool trigger_tile_switch_flag) {
            std::lock_guard<std::mutex> lock(self.mutex_);
            auto &dev = const_cast<typename RPU::AbstractRPUDeviceCuda<T> &>(
                self.getRPUDeviceCuda()
            );
            dev.set_flags_cpp(trigger_tile_switch_flag);
        },
        py::arg("trigger_tile_switch_flag"),
        R"pbdoc(
            Set internal CUDA tile flags (e.g., trigger_tile_switch).
        )pbdoc");    
};
#undef NAME
