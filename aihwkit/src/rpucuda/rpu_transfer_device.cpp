...
void TransferRPUDevice<T>::populate(
  ...
  current_tile_idx_ = this->n_devices_ - 1;
}
...
void TransferRPUDevice<T>::finishUpdateCycle(
  ...
  int current_idx = this->current_update_idx_;
  const auto &par = this->getPar();
  bool transferred = false;

  // ---------- Stage 1: Loss-based tile switching (Warm Start) ----------
  if (trigger_tile_switch_flag_ && current_tile_idx_ >= 0) {
    current_tile_idx_--;
    std::cout << "[TILE SWITCH] -> switched to tile index: " << current_tile_idx_ << std::endl;
    trigger_tile_switch_flag_ = false;

    if (current_tile_idx_ < 0) {
      tile_switch_rounds_done_++;
      std::cout << "[TILE ROUND] -> finished round #" << tile_switch_rounds_done_ << std::endl;
      if (tile_switch_rounds_done_ < max_tile_switch_rounds_) {
        current_tile_idx_ = this->n_devices_ - 1;
        std::cout << "[TILE ROUND] -> restarting from tile " << current_tile_idx_ << std::endl;
      }
    }
  }

  // ---------- Stage 2: Sustain transfer from current tile to tile 0 ----------
  if (tile_switch_rounds_done_ < max_tile_switch_rounds_ &&
      current_tile_idx_ >= 0 &&
      current_idx % getTransferEvery(1, m_batch_info, up) == 0) {
    transfer(current_tile_idx_, 0, current_lr, m_batch_info);
    transferred = true;
  }

  // ---------- Stage 3: Periodic geometric transfer (Residual Learning) ----------
  if (tile_switch_rounds_done_ >= max_tile_switch_rounds_) {
    for (int j = 0; j < this->n_devices_; j++) {
      ...
      if (current_idx % every == 0) {
        transfer(std::min(j + 1, this->n_devices_ - 1), j, current_lr, m_batch_info);
      }
    }
  }
