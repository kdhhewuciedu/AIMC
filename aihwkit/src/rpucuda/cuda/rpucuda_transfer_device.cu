...
void TransferRPUDeviceCuda<T>::runUpdateKernel(
  ...
  if (next_transfer >= m_batch + this->current_update_idx_) {
    ...
    if (trigger_tile_switch_flag_ && current_tile_idx_ >= 0)
       {
        current_tile_idx_--;
        std::cout << "[TILE SWITCH] -> switched to tile index: "
                  << current_tile_idx_ << std::endl;
        trigger_tile_switch_flag_ = false;
      }
    uint64_t prev_idx = this->current_update_idx_ - m_batch;
    auto next_tile1_transfer = getNextTransfer(prev_idx, getTransferEvery(1, m_batch, up));
    if (current_tile_idx_ >= 0 && next_tile1_transfer <= this->current_update_idx_) 
    {
      transfer(current_tile_idx_, 1, up, lr, blm->getCurrentLR());
    }
    ...
    while (next_transfer <= final_update_idx) {
      ...
      uint64_t prev_idx = this->current_update_idx_ - current_m_batch;
      auto next_tile1_transfer = getNextTransfer(prev_idx, getTransferEvery(1, m_batch, up));
      if (current_tile_idx_ >= 0 && next_tile1_transfer <= this->current_update_idx_) 
      {
        transfer(current_tile_idx_, 1, up, lr, blm->getCurrentLR());
      }
      // if (getTransferEvery(1, m_batch, up) > 0&& (this->current_update_idx_ % getTransferEvery(1, m_batch, up)) == 0&& current_tile_idx_ >= 0) 
      //   transfer(current_tile_idx_, 1, up, lr, blm->getCurrentLR());
      if (current_tile_idx_ < 0) {
        for (int j = 1; j < this->n_devices_ - 1; ++j) {
          auto hop_next = getNextTransfer(
              this->current_update_idx_ - current_m_batch,
              getTransferEvery(j, m_batch, up));
          if (hop_next <= this->current_update_idx_) {
            transfer(j + 1, j, up, lr, 1.0);
          }
        }
      }
